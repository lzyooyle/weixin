...
if ($key = trim(stripslashes($key))) {
	$sk = "%{$key}%";
	$fields = explode(" ", "province name mother kehu_id nickname from_nickname");
	$sfield = array();
...
$list_data = $db->query("select *,(order_date-$time) as remain_time, if(order_date<$today_begin, 1, if(order_date>$today_end, 2, 3)) as order_sort, if(status=1,2, if(status=2,1,0)) as status_1 from $table order by id desc $sqlwhere $sqlgroup $sqlsort limit $offset,$pagesize");
	...
	if ($id == 0) {
		...
		if ($uinfo["part_id"] == 2) {			
			...
			if($li["status"] == 1){
				...
				$op[] = "<a href='/m/patient/patient.edit.php?op=edit&id=$id&go=back' class='op'><button type='button' class='btn btn-info'>编辑</button></a>";
				}
